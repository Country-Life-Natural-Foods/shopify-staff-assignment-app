...
function configureSessionStorage({ Session, isProduction }) {
  ...
  if (isProduction && !redisUrl && !postgresUrl) {
    throw new Error(
      'Production environment requires a persistent session store. Set REDIS_URL (Upstash on Vercel) or DATABASE_URL (Neon/Supabase Postgres). In-memory sessions fail on serverless and will lose all data on cold starts.',
    );
  }
