...
function configureSessionStorage({ Session, isProduction }) {
  ...
  if (redisUrl) {
    ...
    shopifySessionStorage = new PostgreSQLSessionStorage(postgresUrl);
    const pgSession = require('connect-pg-simple')(require('express-session'));
    const { Pool } = require('pg');
    const pool = new Pool({
      connectionString: postgresUrl,
      ssl: postgresUrl.includes('sslmode=require') || isProduction
        ? { rejectUnauthorized: false }
        : undefined,
    });
    pool.on('error', (err) => console.error('[postgres] pool', err));
