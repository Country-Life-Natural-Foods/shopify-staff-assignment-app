...
async function runSync() {
  ...
  // Get access token: Dev Dashboard (client credentials) or legacy static token
  ...
  if (!token) {
    ...
    if (!clientId || !clientSecret) {
      throw new Error(
        'Missing auth: set SHOPIFY_ADMIN_ACCESS_TOKEN (legacy) OR SHOPIFY_CLIENT_ID + SHOPIFY_CLIENT_SECRET (Dev Dashboard app)'
      );
    }
