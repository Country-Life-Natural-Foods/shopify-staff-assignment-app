...
class ShopifyPostgresSessionStorage {
  ...
  // Lazily (re)runs the schema check. A warm Vercel container reuses this
  // instance across requests, so a single failed attempt (Neon compute slow
  // to wake, a transient network blip) must not permanently poison session
  // storage for that container's remaining lifetime — production logs showed
  // exactly that: one failed connection got cached forever as `this.ready`,
  // so every later request on that container replayed the same rejection as
  // an unhandled rejection that left the response hanging until Vercel's
  // 300s ceiling forced a 504 ("the app didn't load"). Clearing `_ready` on
  // failure means the next call gets a fresh attempt instead.
  get ready() {
    if (!this._ready) {
      this._ready = this.ensureSchema().catch((err) => {
        console.error('[postgres] shopify session ensureSchema failed', err);
        this._ready = null;
        throw err;
      });
    }
