...
const { orderCompanyId } = require('./order-attribution');
...
const RECENT_ORDERS_QUERY = `
  query getRecentOrders($first: Int!, $after: String) {
    orders(first: $first, after: $after, query: "created_at:>='2025-01-01'", sortKey: CREATED_AT, reverse: true) {
      edges {
        node {
          id
          createdAt
 ...
 */
/**
 * Company id for rollup-style attribution: PurchasingCompany first, else an
 * unambiguous companyContactProfiles link on the buyer customer.
 */
function isB2bOrder(order) {
  return Boolean(orderCompanyId(order));
}

function ordersInDateRange(orders, startDate, endDate) {
...
function summaryFromB2bOrders(orders, { totalCompanies = 0 } = {}) {
  let revenue = 0;
  let currencyCode = 'USD';
  const companies = new Set();
  for (const order of orders) {
    revenue += parseFloat(order.currentTotalPriceSet?.shopMoney?.amount || '0') || 0;
  ...
  return {
    ...
    totalCompanies,
    currencyCode,
    avgOrderValue: orderCount > 0 ? parseFloat((revenue / orderCount).toFixed(2)) : 0,
  };
}
...
async function buildAnalyticsSummary(client, startDate = null, endDate = null) {
  const [ordersPage, totalCompanies] = await Promise.all([
    paginate(client, RECENT_ORDERS_QUERY, 'orders', 5, 'analytics orders'),
    fetchCompaniesCount(client),
  ]);

  const b2bInRange = ordersInDateRange(ordersPage.nodes.filter(isB2bOrder), startDate, endDate);
  const revenueSummary = summaryFromB2bOrders(b2bInRange, { totalCompanies });
  const trend = calculateRevenueTrend(b2bInRange, 'daily', startDate, endDate);
  const topProduct = calculateTopProduct(b2bInRange, startDate, endDate);
...
async function getProductsAnalytics(client, options = {}) {
  ...
  const filteredOrders = ordersInDateRange(
    ordersResult.nodes.filter(isB2bOrder),
    startDate,
    endDate,
  );
...
function aggregateCompanyMetrics(orders, companyId, startDate = null, endDate = null) {
  ...
  const companyOrders = orders.filter((order) => {
    if (orderCompanyId(order) !== companyId) return false;
