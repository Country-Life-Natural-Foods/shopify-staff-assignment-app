...
async function getCompaniesAnalytics(client, options = {}) {
  const { maxPages = 10, sortBy = 'revenue', sortOrder = 'desc', startDate = null, endDate = null } = options;

  const companiesPage = await paginate(client, COMPANIES_REVENUE_QUERY, 'companies', maxPages, 'analytics companies');
  const analytics = buildCompanyAnalytics(companiesPage.nodes);

  // Apply sorting
  if (sortBy === 'name') {
    analytics.sort((a, b) => (sortOrder === 'asc' ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name)));
  } else if (sortBy === 'createdAt') {
    analytics.sort((a, b) => (sortOrder === 'asc'
      ? new Date(a.createdAt) - new Date(b.createdAt)
      : new Date(b.createdAt) - new Date(a.createdAt)));
  } else {
    // Default: sort by revenue
    analytics.sort((a, b) => (sortOrder === 'asc' ? a.totalSpent - b.totalSpent : b.totalSpent - a.totalSpent));
  }
