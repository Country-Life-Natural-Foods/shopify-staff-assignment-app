...
const { resolveCompanyFromContacts } = require('./order-attribution');
...
function summarizeOrders(orders) {
  ...
  for (const order of orders) {
    ...
    const name = company?.name
      || order.customer?.companyContactProfiles?.find((p) => p?.company?.id === companyId)?.company?.name
      || 'Company';
    const current = byCompany.get(companyId) || { id: companyId, name, orders: 0, revenue: 0 };
    current.orders += 1;
