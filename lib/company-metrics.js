...
// Shop-wide `orders` includes DTC volume, so a created_at range still takes
// tens of thousands of pages to reach 2025. Company.orders is B2B-only.
const HISTORICAL_CUTOFF = '2025-01-01T00:00:00.000Z';
const BACKFILL_CURSOR_VERSION = 2;
...
const BACKFILL_COMPANIES_QUERY = `
  query BackfillCompanyOrders($first: Int!, $after: String) {
...
const BACKFILL_ONE_COMPANY_ORDERS_QUERY = `
  query BackfillOneCompanyOrders($id: ID!, $first: Int!, $after: String) {
    company(id: $id) {
      id
...
function emptyCompanyCursor() {
  return {
    v: BACKFILL_CURSOR_VERSION,
    companiesAfter: null,
    skipUntilAfterCompanyId: null,
    companyId: null,
    ordersAfter: null,
  };
}
...
function parseBackfillCursor(raw) {
  if (!raw) return emptyCompanyCursor();
  try {
    const parsed = JSON.parse(raw);
    if (parsed && parsed.v === BACKFILL_CURSOR_VERSION) {
      return {
        v: BACKFILL_CURSOR_VERSION,
        companiesAfter: parsed.companiesAfter || null,
        skipUntilAfterCompanyId: parsed.skipUntilAfterCompanyId || null,
        companyId: parsed.companyId || null,
        ordersAfter: parsed.ordersAfter || null,
      };
    }
  } catch {
    /* shop-wide orders cursor from the old backfill — restart on companies */
  }
  return emptyCompanyCursor();
}

function serializeBackfillCursor(cursor) {
  return JSON.stringify({
    v: BACKFILL_CURSOR_VERSION,
    companiesAfter: cursor.companiesAfter || null,
    skipUntilAfterCompanyId: cursor.skipUntilAfterCompanyId || null,
...
function reachedHistoricalCutoff(nodes) {
  if (!nodes.length) return false;
  return nodes[nodes.length - 1].createdAt < HISTORICAL_CUTOFF;
}
...
function createPostgresMetricsStore(connectionString, isProduction, { getOfflineGraphqlClient } = {}) {
  ...
  async function backfillChunk(shopDomain, graphqlClient, options = {}) {
    ...
    const companyPageSize = Math.min(25, Math.max(3, options.companyPageSize || 3));
    ...
    try {
      while (pages < maxPages && Date.now() - started < maxMs) {
        if (cursor.companyId && cursor.ordersAfter) {
          const data = await shopifyGraphql(
            graphqlClient,
            BACKFILL_ONE_COMPANY_ORDERS_QUERY,
            { id: cursor.companyId, first: orderPageSize, after: cursor.ordersAfter },
            'company metric backfill orders',
          );
          const conn = data?.company?.orders;
          const nodes = orderNodesFromConnection(conn);
          ingested += await ingestNodes(nodes, cursor.companyId);
          pages += 1;
          noteProgress();
          if (!conn?.pageInfo?.hasNextPage || reachedHistoricalCutoff(nodes)) {
            cursor.skipUntilAfterCompanyId = cursor.companyId;
        ...
        const data = await shopifyGraphql(
          graphqlClient,
          BACKFILL_COMPANIES_QUERY,
          { first: companyPageSize, after: cursor.companiesAfter },
          'company metric backfill companies',
        );
        const conn = data?.companies;
        const companyEdges = conn?.edges || [];
        pages += 1;

        let pauseOnCompany = null;
        let skipping = Boolean(cursor.skipUntilAfterCompanyId);
        for (const edge of companyEdges) {
          const company = edge?.node;
          if (!company?.id) continue;
          if (skipping) {
            if (company.id === cursor.skipUntilAfterCompanyId) {
              skipping = false;
              cursor.skipUntilAfterCompanyId = null;
            }
            continue;
          }
          const orders = company.orders;
          const nodes = orderNodesFromConnection(orders);
          ingested += await ingestNodes(nodes, company.id);
          if (orders?.pageInfo?.hasNextPage && !reachedHistoricalCutoff(nodes)) {
            pauseOnCompany = {
              companyId: company.id,
              ordersAfter: orders.pageInfo.endCursor,
            };
            break;
          }
        }
        ...
        if (pauseOnCompany) {
          cursor.companyId = pauseOnCompany.companyId;
          cursor.ordersAfter = pauseOnCompany.ordersAfter;
          cursor.skipUntilAfterCompanyId = pauseOnCompany.companyId;
          continue;
        }
        ...
        cursor.companiesAfter = conn.pageInfo.endCursor;
        cursor.skipUntilAfterCompanyId = null;
        cursor.companyId = null;
        cursor.ordersAfter = null;
      }
