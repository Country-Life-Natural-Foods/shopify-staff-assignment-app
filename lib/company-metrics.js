...
// Shopify's orders connection only returns ~60 days unless `query` sets a created_at range.
const HISTORICAL_ORDER_SEARCH = "created_at:>='2025-01-01'";

const BACKFILL_ORDERS_QUERY = `
  query BackfillB2bOrders($first: Int!, $after: String, $query: String) {
    ...
    try {
      while (pages < maxPages && Date.now() - started < maxMs) {
        const data = await shopifyGraphql(
          ...
          { first: pageSize, after: cursor, query: HISTORICAL_ORDER_SEARCH },
        ...
        try {
          ...
          for (const snap of snaps) {
            const result = await applySnapshot(db, shop, snap);
            if (result.changed && snap.companyId) ingested += 1;
          }
          await db.query('COMMIT');
        } catch (err) {
          await db.query('ROLLBACK');
