...
function createPostgresMetricsStore(connectionString, isProduction, { getOfflineGraphqlClient } = {}) {
    ...
    try {
      ...
      return {
        ...
        productsReady: meta?.product_backfill_status === 'complete'
          || (meta?.backfill_status === 'complete'
            && (meta.backfill_version || 0) >= BACKFILL_CURSOR_VERSION
            && (await hasProductLines(shop))),
  ...
  async function claimBackfill(shop, { rebuild } = {}) {
    ...
    if (rebuild) {
      ...
      await pool.query(
        `INSERT INTO ${META_TABLE} (shop, backfill_status, backfill_cursor, backfill_error, ingested_orders, backfill_started_at, product_backfill_status, product_backfill_cursor, backfill_version)
         VALUES ($1, 'running', NULL, NULL, 0, now(), 'running', NULL, $2)
    ...
    const { rows } = await pool.query(
      `INSERT INTO ${META_TABLE} (shop, backfill_status, backfill_started_at, ingested_orders, backfill_version)
