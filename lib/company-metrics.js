...
// Shopify's order search has no B2B-only filter, so paging `orders` walks every
// retail order since HISTORICAL_CUTOFF and throws most of them away. Company
// orders are already the B2B set: one company at a time, newest first, stop
// when a page crosses the cutoff. Rows already stored stay; a cursor version
// bump only restarts the walk and upserts.
const HISTORICAL_CUTOFF = '2025-01-01T00:00:00.000Z';
const BACKFILL_CURSOR_VERSION = 8;
const BACKFILL_LEASE_STALE = '2 minutes';
...
const BACKFILL_COMPANY_QUERY = `
  query BackfillCompany($after: String) {
    companiesCount {
      count
    }
    companies(first: 1, after: $after) {
      edges {
        cursor
        node {
          id
...
const BACKFILL_COMPANY_ORDERS_QUERY = `
  query BackfillCompanyOrders($id: ID!, $first: Int!, $after: String) {
    company(id: $id) {
      id
...
function parseBackfillCursor(raw) {
  ...
  try {
    ...
    if (parsed && parsed.v === BACKFILL_CURSOR_VERSION) {
      const total = Number(parsed.companiesTotal);
      return {
        v: BACKFILL_CURSOR_VERSION,
        companyAfter: parsed.companyAfter || null,
        companyId: parsed.companyId || null,
        ordersAfter: parsed.ordersAfter || null,
        companiesDone: Number(parsed.companiesDone) || 0,
        companiesTotal: Number.isFinite(total) && total > 0 ? total : null,
        companiesHasNext: parsed.companiesHasNext !== false,
      };
    }
  } catch {
    /* older backfill cursors — restart on the company walk */
  }
...
function serializeBackfillCursor(cursor) {
  return JSON.stringify({
    v: BACKFILL_CURSOR_VERSION,
    companyAfter: cursor.companyAfter || null,
    companyId: cursor.companyId || null,
    ordersAfter: cursor.ordersAfter || null,
    companiesDone: cursor.companiesDone || 0,
    companiesTotal: cursor.companiesTotal || null,
    companiesHasNext: cursor.companiesHasNext !== false,
  });
}
...
function estimateBackfillProgress(meta) {
  ...
  const total = cursor.companiesTotal || 0;
  const done = cursor.companiesDone || 0;
  const ratio = total > 0 ? done / total : 0;
  const percent = Math.min(99, Math.max(0, Math.round(ratio * 100)));
  const phase = cursor.companyId ? 'company' : 'companies';
...
async function applySnapshotsBatch(client, shop, snaps) {
  ...
  const [{ rows: prevOrders }, { rows: refundRows }, { rows: prevLines }] = await Promise.all([
    client.query(
      `SELECT order_id, company_id, day, revenue FROM ${ORDER_TABLE} WHERE shop = $1 AND order_id = ANY($2::text[])`,
      [shop, orderIds],
    ),
    client.query(
  ...
  for (const [orderId, snap] of byOrderId) {
    ...
    if (!orderUnchanged) {
      ...
      orderUpserts.push({ orderId, companyId: nextCompany, createdAt: snap.createdAt, day, revenue: nextRevenue });
      changedCount += 1;
    }
  ...
  if (orderUpserts.length) {
    await client.query(
      `INSERT INTO ${ORDER_TABLE} (shop, order_id, company_id, created_at, day, revenue)
       SELECT $1, u.order_id, u.company_id, u.created_at::timestamptz, u.day::date, u.revenue
       FROM unnest($2::text[], $3::text[], $4::timestamptz[], $5::date[], $6::numeric[])
         AS u(order_id, company_id, created_at, day, revenue)
       ON CONFLICT (shop, order_id) DO UPDATE SET
         company_id = EXCLUDED.company_id,
         created_at = EXCLUDED.created_at,
         day = EXCLUDED.day,
...
function createPostgresMetricsStore(connectionString, isProduction, { getOfflineGraphqlClient } = {}) {
  ...
  function ensureReady() {
    if (!readyPromise) {
      readyPromise = pool.query(`
        ...
        ALTER TABLE ${META_TABLE} ADD COLUMN IF NOT EXISTS backfill_lease text;
  ...
  async function claimBackfill(shop, { rebuild } = {}) {
    ...
    if (rebuild) {
      ...
      await pool.query(
        `INSERT INTO ${META_TABLE} (shop, backfill_status, backfill_cursor, backfill_error, ingested_orders, backfill_started_at, product_backfill_status, product_backfill_cursor, backfill_version, backfill_window_started_at, backfill_lease)
         VALUES ($1, 'running', NULL, NULL, 0, now(), 'running', NULL, $2, now(), $3)
         ON CONFLICT (shop) DO UPDATE SET
           ...
           backfill_lease = EXCLUDED.backfill_lease`,
        [shop, BACKFILL_CURSOR_VERSION, lease],
      );
      return { claimed: true, cursor: null, ingested: 0, lease };
    }

    const { rows } = await pool.query(
      `INSERT INTO ${META_TABLE} (shop, backfill_status, backfill_started_at, ingested_orders, backfill_version, backfill_window_started_at, backfill_lease)
       VALUES ($1, 'running', now(), 0, $2, now(), $3)
       ON CONFLICT (shop) DO UPDATE SET
         backfill_status = 'running',
         backfill_started_at = now(),
         backfill_lease = EXCLUDED.backfill_lease,
       ...
       WHERE (
         ...
         AND (
           ...
           OR ${META_TABLE}.backfill_started_at < now() - interval '${BACKFILL_LEASE_STALE}'
         )
       RETURNING backfill_cursor, ingested_orders, backfill_status, backfill_lease`,
      [shop, BACKFILL_CURSOR_VERSION, lease],
    );
    if (!rows[0]) {
      const meta = await loadMeta(shop);
    ...
    return {
      claimed: true,
      cursor: rows[0].backfill_cursor || null,
      ingested: rows[0].ingested_orders || 0,
      lease: rows[0].backfill_lease || lease,
    };
  }
    ...
    async function holdLease() {
      const held = await pool.query(
        `UPDATE ${META_TABLE}
         ...
         WHERE shop = $1 AND backfill_lease = $2`,
        [shop, lease],
      );
    ...
    try {
      while (pages < maxPages && Date.now() - started < maxMs) {
        ...
        if (!cursor.companyId) {
          const data = await shopifyGraphql(
            graphqlClient,
            BACKFILL_COMPANY_QUERY,
            { after: cursor.companyAfter },
            'company metric backfill company',
          );
          if (cursor.companiesTotal == null && data?.companiesCount?.count != null) {
            cursor.companiesTotal = data.companiesCount.count;
          }
          const edge = data?.companies?.edges?.[0];
          pages += 1;
          console.log('[company-metrics] backfill company', {
            shop,
            done: cursor.companiesDone,
            total: cursor.companiesTotal,
            companyId: edge?.node?.id || null,
          });
          if (!edge?.node?.id) {
            complete = true;
            break;
          }
          cursor.companyId = edge.node.id;
          cursor.companyAfter = edge.cursor || data.companies.pageInfo?.endCursor || cursor.companyAfter;
          cursor.ordersAfter = null;
          cursor.companiesHasNext = Boolean(data?.companies?.pageInfo?.hasNextPage);
          continue;
        }

        const data = await shopifyGraphql(
          graphqlClient,
          BACKFILL_COMPANY_ORDERS_QUERY,
          {
            id: cursor.companyId,
            first: orderPageSize,
            after: cursor.ordersAfter,
          },
          'company metric backfill company orders',
        );
        const conn = data?.company?.orders;
        const nodes = orderNodesFromConnection(conn);
        const kept = [];
        let passedCutoff = false;
        for (const node of nodes) {
          if (!node?.createdAt || node.createdAt < HISTORICAL_CUTOFF) {
            passedCutoff = true;
            break;
          }
          kept.push(node);
        }
        const added = await ingestNodes(kept, cursor.companyId);
        ingested += added;
        pages += 1;
        console.log('[company-metrics] backfill page', {
          ...
          kept: kept.length,
          b2b: added,
          newest: nodes[0]?.createdAt || null,
          oldest: nodes[nodes.length - 1]?.createdAt || null,
        });
        noteProgress();

        const moreOrders = Boolean(conn?.pageInfo?.hasNextPage) && !passedCutoff;
        if (moreOrders) {
          cursor.ordersAfter = conn.pageInfo.endCursor;
          continue;
        }

        cursor.companiesDone += 1;
        cursor.companyId = null;
        cursor.ordersAfter = null;
        if (!cursor.companiesHasNext) {
          complete = true;
          break;
        }
      }
      ...
      if (complete) {
        ...
        const recentWindowMs = Date.now() - (70 * 24 * 60 * 60 * 1000);
        // read_orders without read_all_orders only returns ~60 days. A finished
        // walk whose oldest order is still inside that window did not reach 2025.
        if (coverage.orderCount > 0 && Number.isFinite(oldestMs) && oldestMs > recentWindowMs) {
          console.warn('[company-metrics] backfill only saw the recent order window; not marking complete', {
            shop,
      ...
      if (lostLease) {
        return { enabled: true, ready: false, status: 'running', ingested, pages, lostLease: true };
      }
      ...
      const saved = await pool.query(
        `UPDATE ${META_TABLE}
         ...
         WHERE shop = $1 AND backfill_lease = $6`,
        [
          shop,
      ...
      return {
        ...
        ingested,
        pages,
        companiesDone: cursor.companiesDone,
        companiesTotal: cursor.companiesTotal,
      };
    } catch (err) {
      await pool.query(
        `UPDATE ${META_TABLE}
         SET backfill_status = 'error', backfill_error = $2, last_backfill_at = now(), backfill_lease = NULL
         WHERE shop = $1 AND backfill_lease = $3`,
        [shop, err.message, lease],
      );
      throw err;
    }
  }
